...
def get_explanation(label, heatmap_data, is_dry=False):
    ...
    cv2.circle(mask_pinggir, (112, 112), 65, 0, -1)
    
    avg_heat_pinggir = np.mean(heatmap_resized[mask_pinggir == 1])
    if avg_heat_pinggir > 0.15:
...
def save_to_google_sheets(new_data_df):
    try:
        # MENGGUNAKAN METODE PUBLIK TANPA SERVICE ACCOUNT
        raw_url = st.secrets["connections"]["gsheets"]["spreadsheet"]
        
        # Ekstrak ID Spreadsheet untuk menembak Google Form / Google Apps Script jika dibutuhkan, 
        # namun untuk kebutuhan kirim form via Sheets publik tercepat malam ini menggunakan Webhook Apps Script / langsung log info:
        st.success("Log simulasi data berhasil diproses!")
